...
layui.define('jquery', function(exports){
  ...
  exports('code', function(options){
    ...
    layui.each(elems.reverse(), function(index, item){
      ...
      /**
       * Tinymeng start
       * 识别code的class="lang-php"语言
       * @type {undefined}
       */
      options.title = undefined;
      var classAll = othis.attr("class");
      if(classAll !== undefined){
          var class_array = classAll.split(" "); //字符分割
          for (var i=0;i<class_array.length ;i++ ) {
              if(class_array[i].match(/lang-\w+/).length > 0){
                  options.title = class_array[i].replace('lang-','');
                  break;
              }
          }
      }
