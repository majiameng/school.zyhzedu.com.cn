...
class Upload {
    ...
    private $config = array(
        'mimes'         =>  array('image/jpeg', 'image/gif', 'image/png',  'application/msword', 'application/vnd.ms-office', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','application/zip','application/x-rar','application/octet-stream'), //允许上传的文件MiMe类型
        'maxSize'       =>  0, //上传的文件大小限制 (0-不做限制)
        'exts'          =>  array('jpg', 'gif', 'png', 'jpeg', 'txt', 'doc', 'docx', 'xlsx', 'xls'), //允许上传的文件后缀
    ...
    public function upload($files='') {
		...
		if(!is_dir($this->rootPath)){
           $this->uploader->mkdir($this->rootPath);
        }
		 // && is_writable($rootpath))
        /* 检测上传根目录 */
