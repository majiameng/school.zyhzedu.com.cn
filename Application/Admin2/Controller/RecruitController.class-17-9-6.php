...
class RecruitController extends AdminBaseController {
	...
	*/	
	public function exam(){
		...
		$map['unit']= 1;
		$map['job']= 2;
		$list1 = D('Resume')->where($map)->order('cast(no AS signed) asc')->field('id,no,unit,userid')->select();
		$map['job']= 1;
		$list2 = D('Resume')->where($map)->order('cast(no AS signed) asc')->field('id,no,unit,userid')->select();	
		$map['job']= 5;
		$list3 = D('Resume')->where($map)->order('cast(no AS signed) asc')->field('id,no,unit,userid')->select();	
				
		$where['unit'] = array('neq',1);		
		$list4 = D('Resume')->where($where)->order('unit asc,cast(no AS signed) asc')->field('id,no,unit,userid')->select();
