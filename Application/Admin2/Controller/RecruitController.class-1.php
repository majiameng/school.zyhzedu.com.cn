    ...
    private function _list($is_del = 0){    	
		...
		if($tt!=1) {
			$list = $model->where($where)
				...
				->field('s.score,r.name,r.id,r.userid,r.sex,r.no,r.exam_no,r.unit,r.job,r.school,r.is_join')
				->select(); 
		}else{
			$list = $model->where($where)
				->limit($page->firstRow.','.$page->listRows)
				->order('cast(exam_no as signed) asc,cast(no as signed) asc')
				->field('name,id,userid,sex,no,exam_no,birth,unit,job,school,education,profession,full_time,is_huibi,profession,status,addtime,is_ok,is_lock,is_join')
				->select(); 
		}
