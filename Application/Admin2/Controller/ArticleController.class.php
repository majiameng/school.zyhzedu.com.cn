...
class ArticleController extends AdminBaseController {	
	...
	function _initialize() {
		...
		$this->data_model = M('Article_data');//副表
		$this->cat_model = D('ArticleCategory');
	}	
    ...
    private function _list($is_del = 0){
    	...
    	$cat_option = $this->cat_model->getTree(0,$cat_id);
    ...
    public function edit(){
        ...
        //这里我直接同时查两张表
        $data=$this->model->table('__ARTICLE__ a,__ARTICLE_DATA__ b')->where('a.id=b.id and a.id='.$id)->find();
        $data['create_time']=date('Y-m-d H:i:s',$data['create_time']);
        
        $data['image']=json_decode($data['image']);
        //得到分类的下拉列表  getTree方法在category模型类
        $cat_option = $this->cat_model->getTree(0,$data['category_id']);
        $this->assign('cat_option', $cat_option);
