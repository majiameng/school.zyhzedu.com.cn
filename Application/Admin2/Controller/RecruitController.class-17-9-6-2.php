...
class RecruitController extends AdminBaseController {
    ...
    private function _list($is_del = 0){
		...
		->order('exam_no asc,unit asc,cast(no as signed) asc')
		->field('name,id,sex,no,exam_no,birth,unit,job,school,education,profession,full_time,is_huibi,profession,status,addtime,is_ok,is_lock')
    	->select(); 		
		$map = [];
	...
	public function detail(){		
		...
		if(IS_AJAX) {
			...
			if($status==1) $data['no']=D('Recruit')->getCardNo($info['unit'],$info['job']);
			 //&& empty($info['no'])
			$data['status'] = $status;
	...
	*/	
	public function exam(){
		...
		$map['unit']= 1;
		$map['job']= 2;
			$list1 = D('Resume')->where($map)->order('cast(no AS signed) asc')->field('id,no,userid')->select();
		$map['job']= 1;
		$list2 = D('Resume')->where($map)->order('cast(no AS signed) asc')->field('id,no,userid')->select();	
		
		$arry = array_merge($list1,$list2);
		$arry = array_column($arry,null,'id');
		$where['id'] = array('not in',array_keys($arry));
		
		$list3 = D('Resume')->where($where)->order('cast(no AS signed) asc')->field('id,no,userid')->select();
	...
	*/
    public function excel()
    {
		...
		$obj->getExcelBao($info,$cnTable,'12');
		exit;		
    }
	...
	*/
    public function nameList($is_del = 0){
		...
		if(isset($_GET['do'])) {
			...
			$data=$model->order('exam_no asc,unit asc,cast(no AS signed) asc')->where($where)->field('id,userid,exam_no,no,unit')->select();
	...
	/*
	** optionsradiosinsel 1 文科；0 理科
	** type 42 天津班;43高三；44夏令营
	** cur 1 应届生；0复读生
	*/
