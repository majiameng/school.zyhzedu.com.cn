...
class RecruitModel extends Model{
	...
	public function getJob(){
		$job = F('_common/job');
		if(empty($job)) $job = $this->saveJobCache();
		return $job;
	}
	...
	public function getCompany(){
		...
		if(empty($company)) $company = $this->saveCompanyCache();
		return $company;
	}
	...
	*/
	public function saveCompanyCache(){
		...
		D('Admin2/Recruit')->saveCityCompanyCache($company);
		return $company;
	}
	/*
	** 获取版本信息
	*/
	public function getVersion(){		
		$info = F('_common/version');
		if(empty($info)) $info = $this->updateVersion();
		return $info;
	}
